...
import { Loader2, FileText, Eye, FileType, AlertCircle, Code, RefreshCw, List, ExternalLink, Quote, Undo } from 'lucide-react'
          ...
          {wordEnabled && (
            <>
              ...
              <button
                ...
                className="flex items-center gap-1 px-2 py-0.5 text-[11px] rounded transition-colors"
                style={{ background: 'var(--na-accent-soft)', color: 'var(--na-accent)' }}
                title="解压并编辑 document.xml"
              >
                {isUnpacking ? <Loader2 className="w-3 h-3 animate-spin" /> : <Code className="w-3 h-3" />}
                编辑 XML
              </button>
            </>
          )}
