...
export default function WordViewer({ filePath }: WordViewerProps) {
  ...
  /** Return the current scrollTop of the scrollable parent container. */
  const getScrollTop = useCallback((): number | undefined => {
   ...
   * If restoreScrollTop is provided, restore that exact scroll position after rendering.
   */
  const loadDocument = useCallback(async (restoreScrollTop?: number, showLoading = true) => {
    ...
    try {
      ...
      if (containerRef.current) {
        let globalIdx = 0
        const allParagraphs = containerRef.current.querySelectorAll('article p')
        allParagraphs.forEach((p) => {
          p.setAttribute('data-p-index', String(globalIdx++))
        })
      }
  ...
  useEffect(() => {
    ...
    const handleBlur = () => {
      ...
      window.electronAPI.wordReplaceParagraph(filePath, editingParagraph, newText)
        .then((result) => {
          if (result.success) {
            // Update the paragraph text in-place without re-rendering the whole
            // document (avoids flash).  We keep the first <span> child which
            // carries the font-size / font-family styles, and discard any extra
            // siblings so the text doesn't get duplicated.
            const container = containerRef.current
            if (container) {
              const pEl = container.querySelector(`[data-p-index="${editingParagraph}"]`)
              if (pEl) {
                const firstSpan = pEl.querySelector('span')
                if (firstSpan) {
    ...
    <div className="flex flex-col h-full w-full" style={{ background: 'var(--na-bg-panel)' }}>
      ...
      >
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            onClick={() => loadDocument(getScrollTop())}
            disabled={isLoading}
            className="flex items-center gap-1 px-2 py-0.5 text-[11px] rounded transition-colors"
      ...
      <div className="flex-1 overflow-hidden relative">
        ...
        {contextMenu?.visible && (
          ...
          >
            ...
            <button
              onClick={() => { loadDocument(getScrollTop()); setContextMenu(null) }}
              className="flex items-center gap-2 px-3 py-1.5 text-[12px] text-left transition-colors hover:bg-[var(--na-bg-hover)]"
              style={{ color: 'var(--na-text-secondary)' }}
            >
